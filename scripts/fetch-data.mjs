...
async function main() {
  ...
  // A rate-limited or failed source must not erase data a previous run fetched:
  // carry forward any dataset this run could not produce (GATC 429s, Alphix
  // hiccups). A dataset only updates when its pull actually succeeds.
  try {
    const prev = readFileSync(OUT, 'utf8');
    const m = prev.match(/window\.MI_REMOTE = ([\s\S]*);\s*$/);
    if (m) {
      const old = JSON.parse(m[1]);
      for (const k of Object.keys(old)) if (!(k in remote)) {
        remote[k] = old[k];
        console.log('Carried forward', k, 'from the previous data.js (source unavailable this run).');
      }
    }
  } catch {}
